...
export class VerystreamScraper extends SourceRunnerScraper<IVerystreamScraperSourceData> {
    public name: string = 'verystream';
    public domains: string[] = ['verystream.com'];
    public urlPattern: RegExp = /(?:(?:https?:)?\/\/)?(?:www\.)?(verystream\.com)\/(e|stream)\/(\w+)(\/.*)?/i;
    ...
    protected async execWithArgs({ page }: IPuppeteerRunnerArgs): Promise<IVerystreamScraperSourceData> {

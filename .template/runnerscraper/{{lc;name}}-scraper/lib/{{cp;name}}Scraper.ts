import { IRunnerScraperOptions, {{cp;type}}RunnerScraper, I{{cp;type}}Data, {{cp;type}} } from 'source-scraper-core';
...
export class {{cp;name}}Scraper extends {{cp;type}}RunnerScraper<I{{cp;name}}Scraper{{cp;type}}Data> {
    public name: string = '{{lc;name}}';
    public domains: string[] = [{{domains}}];
    public urlPattern: RegExp = /(?:(?:https?:)?\/\/)?(?:[^.]+\.)?{{urlPattern}}/i;
    public runner: {{cp;runner}}Runner<I{{cp;name}}Scraper{{cp;type}}Data> = new {{cp;runner}}Runner<I{{cp;name}}Scraper{{cp;type}}Data>();
    public defaultOptions: I{{cp;name}}ScraperOptions = {};

    protected async execWithArgs(args: I{{cp;runner}}RunnerArgs): Promise<I{{cp;name}}Scraper{{cp;type}}Data> {
